...
export const getProducts = async (categoryFilter?: string, resultLimit?: number): Promise<Product[]> => {
  ...
  const products = querySnapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) } as Product));
...
export const addProduct = async (productData: Omit<Product, 'id'>): Promise<Product> => {
  const dataToSave = { ...productData, createdAt: new Date().toISOString() };
...
// ANALYTICS & USERS
export const addGuestVisit = async (): Promise<void> => {
  await addDoc(collection(db, 'guestVisits'), {
    timestamp: new Date().toISOString()
  });
};

export const getGuestVisits = async (): Promise<any[]> => {
  const querySnapshot = await getDocs(collection(db, 'guestVisits'));
  return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
};

export const addProductView = async (productId: string): Promise<void> => {
...
export const getProductViews = async (): Promise<any[]> => {
  const querySnapshot = await getDocs(collection(db, 'productViews'));
  return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
};
