...
import { getProductById, addProductView } from '../services/db';
...
function ProductDetails() {
  ...
  useEffect(() => {
    ...
    getProductById(id)
      .then(data => {
        ...
        setLoading(false);
        addProductView(data.id).catch(console.error);
      })
      .catch(err => {
        console.error(err);
        setLoading(false);
  ...
  const handleAddToCart = () => {
    if (!product) return;
    if (!user) {
      localStorage.setItem('pending_action', JSON.stringify({ type: 'add_to_cart', product }));
      navigate('/login');
      return;
    }
    addToCart(product);
  ...
  const handleOrderNow = (e: React.MouseEvent) => {
    ...
    if (!user) {
      localStorage.setItem('pending_action', JSON.stringify({ type: 'checkout', productId: product.id }));
      navigate('/login');
      return;
    }
