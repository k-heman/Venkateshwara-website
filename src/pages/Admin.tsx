...
import { PlusCircle, Edit, Trash2, Box, Layers, X, Loader2, Check, AlertCircle, BarChart3 } from 'lucide-react';
...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'categories' | 'statistics'>('statistics');
  ...
  const toggleShowOnHome = async (product: Product) => {
    try {
      await updateProduct(product.id, { showOnHome: !product.showOnHome });
      fetchData();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="section container animate-fade-in" style={{ padding: '3rem 1.5rem', minHeight: '80vh' }}>
      ...
      <div className="flex mb-8" style={{ gap: '1rem', borderBottom: '2px solid #e2e8f0', paddingBottom: '1rem', marginBottom: '2rem', overflowX: 'auto' }}>
        <button onClick={() => setActiveTab('statistics')} className={`btn ${activeTab === 'statistics' ? 'btn-primary' : 'btn-outline'}`} style={{ whiteSpace: 'nowrap' }}><BarChart3 size={20} /> Statistics</button>
        <button onClick={() => setActiveTab('products')} className={`btn ${activeTab === 'products' ? 'btn-primary' : 'btn-outline'}`} style={{ whiteSpace: 'nowrap' }}><Box size={20} /> Manage Products</button>
        <button onClick={() => setActiveTab('categories')} className={`btn ${activeTab === 'categories' ? 'btn-primary' : 'btn-outline'}`} style={{ whiteSpace: 'nowrap' }}><Layers size={20} /> Manage Categories</button>
      </div>

      {activeTab === 'products' && (
        <div className="flex flex-col animate-fade-in" style={{ gap: '2rem' }}>
          {/* Category Filter */}
          <div className="flex-between bg-white p-4 rounded-lg shadow-sm" style={{ padding: '1rem', borderRadius: '0.5rem', background: 'white', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
             <h2 className="heading-sm">Products List</h2>
             <div className="flex-center" style={{ gap: '1rem' }}>
               <label className="text-sm font-semibold text-muted">Filter by Category:</label>
               <select className="input" style={{ width: '250px' }} value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', borderRadius: '0.5rem', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
              <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', textAlign: 'left' }}>
                <tr>
                  ...
                  <th style={{ padding: '1rem', textAlign: 'center' }}>Home</th>
                  <th style={{ padding: '1rem', textAlign: 'right' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredProducts.length === 0 ? (
                  <tr><td colSpan={7} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>No products found. Add one below!</td></tr>
                ) : (
          ...
          <div className="card glass" style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto', width: '100%' }}>
            ...
            <form onSubmit={handleProductSubmit}>
              ...
              <div className="input-group" style={{ marginBottom: '1.5rem' }}>
                <label className="label">Show Product on Home Page</label>
                <label className="flex items-center gap-3 cursor-pointer p-2 hover:bg-white rounded-lg transition-colors border" style={{ borderColor: '#e2e8f0', borderRadius: '0.5rem', padding: '1rem' }}>
                  <input 
                    type="checkbox" 
                  ...
                  />
                  <span className="text-sm font-semibold text-gray-700">Display this product on the home page for users and guests</span>
                </label>
              </div>
